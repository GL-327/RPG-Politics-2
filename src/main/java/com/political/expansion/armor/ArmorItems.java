...
/**
 * Builds {@code arm_} armour-piece {@link ItemStack}s. Mirrors {@link RpgItems#create} exactly so the
 * pieces plug straight into the existing Skyblock pipeline (stats via {@link ItemStats}, abilities via
 * {@link com.political.combat.AbilityEngine}, decoration/colours via {@link com.political.items.Rarity}).
 *
 * <p><b>Skyblock stats only:</b> like every custom item in this mod, the vanilla attribute modifiers
 * (armour, toughness, attack) are stripped to {@link ItemAttributeModifiers#EMPTY}; all power comes
 * from the {@code custom_data} stat keys read by the stat system.
 */
public final class ArmorItems {

    /** custom_data key marking which set a piece belongs to (used by {@link ArmorSetBonusHandler}). */
    public static final String SET_KEY = "arm_set";
    ...
    public static ItemStack create(ArmorSet set, ArmorSet.Slot slot) {
        ...
        // Custom inventory icon (assets/politicalserver/items/<id>.json -> models/item/<id>.json).
        stack.set(DataComponents.ITEM_MODEL, Identifier.fromNamespaceAndPath(NAMESPACE, id));

        // Skyblock-governed gear: no vanilla armour/toughness/attack modifiers apply or show.
        ...
        stack.set(DataComponents.LORE, new ItemLore(ArmorTooltip.build(stack, set, piece)));

        return stack;
    }

    /** Reads the set key written on a worn piece, or {@code null} if the stack isn't one of ours. */
    ...
    /** Every armour piece, one stack each (for creative tabs / catalogue wiring by integration). */
    public static List<ItemStack> items() {
        List<ItemStack> out = new ArrayList<>();
        for (ArmorSet set : ArmorSet.values()) {
            for (ArmorSet.Slot slot : ArmorSet.Slot.values()) {
                out.add(create(set, slot));
            }
        }
        return out;
    }
