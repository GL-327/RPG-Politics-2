/**
 * Phase-2 global cuisine texture generator (com/political/expansion2/food).
 * Writes assets for every `food2_*` id only — safe alongside Phase-1 food_*.
 *
 * Usage: node tools/gen-food2.js   (requires tools/node_modules with jimp)
 */
...
const P = {
  ...
  purple:    [[50, 20, 70], [90, 44, 120], [130, 72, 168], [180, 120, 210], [220, 180, 240]],
};
const WHITE = [255, 255, 255];

const GARNISH = {
  leaf: P.leaf[2], green: P.leaf[2], red: P.red[2], orange: P.orange[3],
  yellow: P.yellow[3], cream: P.cream[3], brown: P.brown[2], cooked: P.cooked[2],
  pink: P.pink[2],
};
...
// --- Phase-2 additional archetypes ---
draw.skewer = (cv, s) => {
  const p = s.pal || P.cooked;
  line(cv, 4, 13, 12, 5, P.brown[2]);
  for (const [x, y] of [[8, 7], [9, 8], [7, 9], [10, 7]]) sphere(cv, x, y, 2, p);
  px(cv, 8, 7, p[4]);
};

draw.onigiri = (cv, s) => {
  const rice = P.cream, nori = P.nori;
  for (let y = 5; y <= 12; y++) for (let x = 5; x <= 10; x++) {
    if ((x - 7.5) * (x - 7.5) / 8 + (y - 8.5) * (y - 8.5) / 12 <= 1) {
      let c = rice[2]; if (x <= 6) c = rice[3]; if (x >= 9) c = rice[1]; px(cv, x, y, c);
    }
  }
  fillRect(cv, 5, 10, 10, 12, nori[2]); line(cv, 5, 10, 10, 10, nori[3]);
  px(cv, 6, 7, rice[4]);
};
...
draw.flatbread = (cv, s) => {
  ...
  for (let y = 6; y <= 11; y++) for (let x = 3; x <= 12; x++) {
    if ((x - 7.5) * (x - 7.5) / 30 + (y - 8.5) * (y - 8.5) / 8 <= 1) {
      let c = p[2]; if (y <= 7) c = p[3]; if (y >= 10) c = p[1]; px(cv, x, y, c);
    }
  }
  for (const [x, y] of [[6, 8], [9, 9], [11, 7]]) px(cv, x, y, P.brown[1]);
  px(cv, 5, 7, p[4]);
};

draw.croissant = (cv, s) => {
  const p = P.crust;
  const pts = [[4, 10], [5, 8], [7, 6], [9, 5], [11, 6], [12, 8], [13, 10]];
  for (let i = 0; i < pts.length; i++) {
    const [x, y] = pts[i];
    for (let k = -1; k <= 1; k++) px(cv, x + k, y, p[2]);
    px(cv, x, y - 1, p[3]); if (i > 0) line(cv, pts[i - 1][0], pts[i - 1][1], x, y, p[2]);
  }
  px(cv, 9, 5, p[4]);
};
...
draw.baguette = (cv, s) => {
  const p = P.crust;
  for (let i = 0; i <= 10; i++) {
    const x = 3 + i, y = 8 + Math.round(Math.sin(i * 0.5) * 0.5);
    for (let k = -1; k <= 1; k++) px(cv, x, y + k, p[2]);
    px(cv, x, y - 1, p[3]);
  }
  line(cv, 3, 9, 4, 8, p[1]); line(cv, 13, 9, 12, 8, p[1]);
  px(cv, 6, 8, p[4]);
};

draw.lasagna = (cv, s) => {
  const p = P.red, cheese = P.yellow, crust = P.crust;
  fillRect(cv, 3, 6, 12, 12, crust[2]);
  for (let y = 7; y <= 11; y += 2) { fillRect(cv, 4, y, 11, y, p[2]); fillRect(cv, 4, y + 1, 11, y + 1, cheese[3]); }
  px(cv, 5, 7, p[4]);
};

draw.empanada = (cv, s) => {
  const p = P.crust;
  for (let y = 6; y <= 12; y++) for (let x = 4; x <= 11; x++) {
    if ((x - 7.5) * (x - 7.5) / 16 + (y - 9) * (y - 9) / 10 <= 1) {
      let c = p[2]; if (x <= 5) c = p[3]; if (x >= 10) c = p[1]; px(cv, x, y, c);
    }
  }
  line(cv, 4, 9, 11, 9, p[1]);
  px(cv, 6, 8, p[4]);
};

draw.fishChips = (cv, s) => {
  draw.steak(cv, s);
  for (const [x, y] of [[4, 12], [6, 13], [8, 12], [10, 13], [12, 12]]) px(cv, x, y, P.yellow[2]);
};

draw.sausage = (cv, s) => {
  const p = P.cooked;
  for (let x = 4; x <= 12; x++) {
    for (let k = -1; k <= 1; k++) px(cv, x, 9 + k, p[2]);
    px(cv, x, 8, p[3]); px(cv, x, 10, p[1]);
  }
  px(cv, 4, 9, p[1]); px(cv, 12, 9, p[1]);
  px(cv, 7, 8, p[4]);
};

draw.wrap = (cv, s) => {
  const shell = P.crust;
  fillRect(cv, 4, 7, 12, 11, shell[2]);
  fillRect(cv, 5, 6, 11, 7, P.leaf[2]);
  for (const [x, y] of [[6, 8], [9, 8], [8, 9]]) px(cv, x, y, P.red[2]);
  line(cv, 4, 7, 12, 7, shell[3]); line(cv, 4, 11, 12, 11, shell[1]);
  px(cv, 5, 8, shell[4]);
};

draw.plate = (cv, s) => {
  const food = s.pal || P.cooked;
  const rim = P.white;
  for (let y = 8; y <= 13; y++) for (let x = 2; x <= 13; x++) {
    const w = (y >= 12) ? (y - 11) : 0;
    if (x >= 2 + w && x <= 13 - w) { let c = rim[2]; if (x <= 3) c = rim[3]; px(cv, x, y, c); }
  }
  for (let y = 5; y <= 9; y++) for (let x = 4; x <= 11; x++) {
    if ((x - 7.5) * (x - 7.5) / 20 + (y - 8) * (y - 8) / 8 <= 1) px(cv, x, y, food[2]);
  }
  for (const [x, y] of [[5, 7], [9, 6], [10, 8]]) px(cv, x, y, P.leaf[2]);
  px(cv, 5, 6, food[4]);
};

draw.slice = (cv, s) => draw.pie(cv, s);

draw.cannoli = (cv, s) => {
  const p = P.crust, fill = P.cream;
  for (let x = 5; x <= 11; x++) {
    px(cv, x, 8, p[2]); px(cv, x, 9, p[1]); px(cv, x, 7, p[3]);
    if (x >= 9) { px(cv, x, 8, fill[2]); px(cv, x, 7, fill[3]); }
  }
  px(cv, 11, 7, fill[4]); px(cv, 5, 8, p[4]);
};

draw.cupcake = (cv, s) => {
  const base = P.crust, top = P.pink;
  fillRect(cv, 5, 9, 10, 12, base[2]);
  for (let y = 5; y <= 8; y++) for (let x = 5; x <= 10; x++) {
    if ((x - 7.5) * (x - 7.5) / 8 + (y - 7) * (y - 7) / 6 <= 1) px(cv, x, y, top[2]);
  }
  px(cv, 7, 5, top[4]);
};

draw.eclair = (cv, s) => {
  const p = P.crust, fill = P.choco;
  fillRect(cv, 4, 7, 12, 10, p[2]);
  fillRect(cv, 4, 6, 12, 7, fill[2]); line(cv, 4, 6, 12, 6, fill[3]);
  px(cv, 5, 8, p[4]);
};

draw.churros = (cv, s) => {
  const p = P.crust;
  line(cv, 4, 12, 12, 4, p[2]);
  for (const [x, y] of [[4, 12], [6, 10], [8, 8], [10, 6], [12, 4]]) {
    px(cv, x, y, p[3]); px(cv, x + 1, y, p[1]);
  }
  px(cv, 12, 4, P.yellow[3]);
};

draw.hotDog = (cv, s) => {
  const bun = P.crust, dog = P.cooked;
  fillRect(cv, 3, 8, 13, 10, bun[2]);
  fillRect(cv, 4, 7, 12, 8, bun[3]);
  fillRect(cv, 4, 10, 12, 11, bun[1]);
  fillRect(cv, 5, 8, 11, 9, dog[2]);
  px(cv, 5, 8, dog[4]);
};

draw.pretzel = (cv, s) => {
  const p = P.crust;
  line(cv, 5, 8, 8, 5, p[2]); line(cv, 8, 5, 11, 8, p[2]);
  line(cv, 5, 8, 5, 11, p[2]); line(cv, 11, 8, 11, 11, p[2]);
  line(cv, 5, 11, 8, 9, p[2]); line(cv, 11, 11, 8, 9, p[2]);
  px(cv, 8, 5, p[4]); px(cv, 8, 9, p[3]);
};

draw.nachos = (cv, s) => {
  const chip = P.yellow;
  for (const [x, y, r] of [[6, 9, 0], [9, 8, 1], [7, 11, -1], [10, 10, 0]]) {
    for (let i = 0; i < 4; i++) px(cv, x + i + r, y, chip[2]);
    px(cv, x, y, chip[3]);
  }
  for (const [x, y] of [[7, 8], [9, 9]]) px(cv, x, y, P.red[2]);
  px(cv, 8, 7, P.green[3]);
};

draw.quesadilla = (cv, s) => {
  const p = P.crust;
  for (let y = 7; y <= 11; y++) for (let x = 4; x <= 11; x++) {
    let c = p[2]; if (y <= 8) c = p[3]; if (y >= 10) c = p[1]; px(cv, x, y, c);
  }
  line(cv, 4, 9, 11, 9, P.yellow[2]);
  px(cv, 5, 8, p[4]);
};

draw.fries = (cv, s) => {
  const p = P.yellow;
  for (const x of [5, 7, 9, 11]) for (let y = 4; y <= 12; y++) {
    px(cv, x, y, p[2]); px(cv, x - 1, y, p[3]);
  }
  fillRect(cv, 4, 12, 12, 13, P.red[2]);
  px(cv, 7, 5, p[4]);
};

draw.cornDog = (cv, s) => {
  draw.hotDog(cv, s);
  for (let y = 5; y <= 9; y++) for (let x = 6; x <= 10; x++) if ((x + y) % 2 === 0) px(cv, x, y, P.yellow[2]);
};

draw.arepa = (cv, s) => {
  const p = P.crust;
  for (let y = 7; y <= 11; y++) for (let x = 4; x <= 11; x++) {
    if ((x - 7.5) * (x - 7.5) / 18 + (y - 9) * (y - 9) / 6 <= 1) {
      let c = p[2]; if (y <= 8) c = p[3]; px(cv, x, y, c);
    }
  }
  fillRect(cv, 5, 8, 10, 9, P.yellow[3]);
  px(cv, 6, 8, p[4]);
};

draw.heroSandwich = (cv, s) => {
  const bun = P.crust;
  fillRect(cv, 2, 6, 14, 7, bun[3]);
  fillRect(cv, 2, 8, 14, 9, P.leaf[2]);
  fillRect(cv, 2, 9, 14, 10, P.cooked[2]);
  fillRect(cv, 2, 10, 14, 11, P.yellow[3]);
  fillRect(cv, 2, 11, 14, 12, bun[2]);
  px(cv, 3, 7, bun[4]);
};

// ----------------------------------------------------------------------------
// SPECS — built from food2-roster.js; ids prefixed food2_.
// ----------------------------------------------------------------------------
function resolvePal(name) {
  if (!name) return P.brown;
  if (Array.isArray(name)) return name;
  return P[name] || P.brown;
}

function resolveSpec(raw) {
  const spec = Object.assign({}, raw.draw);
  if (typeof spec.pal === 'string') spec.pal = resolvePal(spec.pal);
  if (typeof spec.garnish === 'string') spec.garnish = GARNISH[spec.garnish] || resolvePal(spec.garnish)[2];
  if (typeof spec.straw === 'string') spec.straw = resolvePal(spec.straw)[2];
  if (spec.topping === 'cream') spec.topping = P.cream[4];
  return spec;
}

const FOOD2_SPECS = Object.fromEntries(
  ROSTER.map((e) => [e.id, resolveSpec(e)])
);
...
async function generate() {
  ...
  let n = 0;
  for (const [id, spec] of Object.entries(FOOD2_SPECS)) {
    const cv = canvas();
  ...
  console.log('food2 items:', n);
}

async function buildPreview() {
  fs.mkdirSync(PREVIEW_DIR, { recursive: true });
  const ids = Object.keys(FOOD2_SPECS);
  ...
  await sheet.write(path.join(PREVIEW_DIR, 'food2_montage.png'));
  console.log('preview ->', path.join(PREVIEW_DIR, 'food2_montage.png'));
}
